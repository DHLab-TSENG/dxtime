...
`dataLength_limit`：將資料長度不足的個案於以排除，預設為0。  
`count_limit`：若是診斷紀錄的筆數過少，有可能意味著該個案極少至此醫療單位看診，研究者可設定診斷紀錄的數量下限，來決定是否採納此個案之紀錄，預設為0。  
...
第一種結果`DataFile_pat`，該資料表顯示三個時間點，firstdate、predicdate、indexdate。
firstdate為第一比診斷紀錄的日期；predictdate為預測資料的最後一個預測時間點，也就是indexdate-followindow；indexdate為最後一筆資料的時間點。
dataLength計算該個案在predictdate之前的資料長度，而n計算該個案的診斷紀錄筆數，如符合研究設定的篩選條件(dataLength>dataLength_limit & n>count_limit)的個案，在cri顯示TRUE，反之則為FALSE。  

第二種資料`DataFile_rec`，從符合個案的資料中，篩選出符合研究長度(followindow)的診斷紀錄。  
...
需設定的參數有:  
`getPeriodage`:預設為FALSE，如設定為TRUE，可利用BIRTHDAY計算出的每個時間窗的年齡periodage，以數字型態呈現，若需要以二元形式呈現，可用binaryage=T，預設會以45歲為分界，也可以使用agelayer自行設定。  
`predictwindow`: 預測天數，意指「想要於指標日多久前進行預測」。    
`window`：設定時間窗的長度。  
...
```{r}
# forcut <- merge(length$DataFile_rec %>% filter(ID %in% match$ID),pat_dm[,.(ID,BIRTHDAY)],all.x=T)
...
`window`：設定時間窗的長度。  
`N`：設定時間窗數量，注意，若該個案在某時間窗中沒有診斷資料，將會自動補0。  
`isDescription`：結果呈現的特徵表是否顯示CCS的中文描述。  

```{r}
head(length$DataFile_rec)
#forselect <- length$DataFile_rec[length$DataFile_rec$ID %in% match$ID,]
forselect <- merge(record_dm,pat_dm[,c("ID","label")],by="ID",all.x = T)
...
以第一個table舉例，HR為`100`的風險比率；Pr(>|z|)為p-value；caseCount為該時間窗中共有多少個案患病，最後，若p-value<0.05 & caseCount>casecount_limit會在selected顯示為TRUE。  
...
3. **DataFile_personal**：個案資料，包含ID、label、SEX、age，若有需放入一併建模的特徵，也須於此表中提供欄位；  
另外，需設定的參數有：  
`predictor`：除了CCS之外，若是有個案相關資料須一併做為預測變量進行建模，需於此填入變量名稱。並於DataFile_personal提供該變量資料。  
`method`：分為"per window"以及"one window"兩種方法，兩種方法的差別在於DataFile_feature的不同，若是多時間窗的特徵進行建模，此參數應設為"per window"，若是單時間窗的特徵進行建模則設為"one window"。  

```{r}
record_dm <- record_dm
lengthdata <- unique(record_dm[,dataLength :=max(date)-min(date),by="ID"][,.(ID,dataLength)])
# lengthdata <- unique(length$DataFile_rec[,futime:=max(date)-min(date),by="ID"][,.(ID,futime)])
...
2. **DataFile_personal**：個案資料，包含ID、label、SEX、age，若有需放入一併建模的特徵，也須於此表格提供；  
另外，需設定的參數有：  
`predictor`：除了CCS之外，若是有個案相關資料須一併做為預測變量進行建模，需於此填入變量名稱。並於DataFile_personal提供該變量資料。  
`N`: 時間窗數量
...
# lstm <- analWindow_LSTM(DataFile_cutdata=windowcut,
#                          window_N,
#                          ID,
#                          periodage=NULL,
#                          DataFile_personal=personal,
...
model %>% predict(x_test, y_test, batch_size = 100) %>% head()
```
### 七、視覺化
...
method可分為top和ccslevel，top的預設為前10數量的的分類。
```{r}
head(windowcut)
plotWindow(DataFile_cutdata = windowcut,method="top",LVL_1_LABEL="Neoplasms",topN=10)
```
...
plotHR(DataFile = coxmodel$model_table[[1]],
       pvalue = 0.05)
```

#### 生存區線視覺化  
將視覺化將病例組與對照組的生存曲線轉換成grouped code，進行視覺化。  
