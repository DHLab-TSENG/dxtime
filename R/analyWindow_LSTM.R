analyWindow_LSTM <- function(DataFile_cutdata,window_N,ID,periodage=NULL,DataFile_personal,label,predictor=c(),N,batch_size=100,Epoch=10,units=16){

  #input cutdata
  DataFile_cutdata <- as.data.table(DataFile_cutdata)
  ccscategory <- c(ccsDxICD9$CCS_CATEGORY %>% unique())
  ...
  batch_size = 100
  test_sam <- sample(1:nrow(test))[1:(nrow(test)-nrow(test) %% batch_size)]
  test <- test[test_sam,,]
  ...
  train_T <- train[label==1,,][sam_T[1:nrow(train[label==1])-nrow(train[label==1])%%5]]
  dices_T  <- sample(1:nrow(train_T[label==1]))
  folds_T <- cut(dices_T, breaks = 5, label = FALSE)#train test 8:2
  sam_F <- sample(1:nrow(train[label==0]))
  train_F <- train[label==0,,][sam_F[1:(nrow(train[label==0])-nrow(train[label==0])%%5)]]
  ...
  for(i in 1:k){
    ...
    val_sam <- sample(1:length(val_indices))[1:(length(val_indices)-length(val_indices)%%batch_size)]
    val_data <- x_newtrain[val_indices,,][val_sam,,]
    val_targets <- y_newtrain[val_indices][val_sam]
    partial_indices <- which(folds != i , arr.ind = TRUE)
    pattial_train_sam <- sample(1:length(partial_indices))[1:(length(partial_indices)-length(partial_indices)%%batch_size)]
  ...
  ##tag 0307
  average_auc_history <- data.frame(
    epoch = seq(1:ncol(all_auc_histories)),
    validation_auc = apply(all_auc_histories, 2, mean)
  )
  setDT(average_auc_history)
  epoch = average_auc_history[validation_auc==max(average_auc_history$validation_auc),epoch]
  model %>% fit(
    x = partial_train_data,
    y = partial_train_targets,
    batch_size = batch_size,
