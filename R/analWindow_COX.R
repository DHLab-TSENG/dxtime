analWindow_COX <- function(DataFile_cutdata,window_N,ID,DataFile_feature,DataFile_personal,label,dataLength,predictor=c(),periodAge=NULL,isDescription=F,method,N){
  ...
  dataCol <- c(deparse(substitute(ID)),deparse(substitute(label)),deparse(substitute(dataLength)),predictor)
  DataFile_personal <- DataFile_personal[, dataCol, with=FALSE]
  names(DataFile_personal) <- c("ID","label","dataLength",predictor)
