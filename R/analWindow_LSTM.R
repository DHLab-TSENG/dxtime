analWindow_LSTM <- function(DataFile_cutData,DataFile_personal,labelColName,DataFile_feature=NULL,predictorColName=NULL,N,batch_size=100,Epoch=10,layer=1,layer1_dropout=0,layer1_units=16,layer2_dropout=0,layer2_units=16,testN=100){
  ...
  #DataFile_personal$label <- as.factor(DataFile_personal$label)
  names(DataFile_personal) <- c("ID","label",predictorColName)

  #將個人資料以及ccs的window資料合併成最後要用的資料表
  maintable <- merge(cutdata,DataFile_personal[,c("ID",predictorColName),with=FALSE],by="ID",all.x=T)
  ...
  series <- merge(series, unique(DataFile_personal[,c("ID","label")]),by="ID",all.x=T)
  class(series)
  series <- as.data.frame(series)
  series <- series[,order(colnames(series),decreasing=TRUE),]#series??data.frame?A?~?|?O?諸===
  setDT(series)
  #series <- series[,c(3:ncol(series)):=lapply(.SD,as.factor),,.SDcols=3:ncol(series)]
  ...
  for(j in 1:testN){
    ...
    x_newtrain <- array(unlist(x_newtrain),dim=c(nrow(newtrain),N,(ncol(newtrain)-2)/N))#unlist
    ...
    test <- test[test_sam,,]
    test_targets = array(unlist(test[, 1]))
    x_test = test[, 3:ncol(test)]
    test_data <- array(unlist(x_test),dim=c(nrow(test),N,(ncol(test)-2)/N))
    ...
    test_evaluation <- model %>% evaluate(test_data, test_targets, batch_size = batch_size)
    test_average_auc[j] <- test_evaluation[[2]]
    train_evaluation <- model %>% evaluate(train_data, train_targets, batch_size = batch_size)
    model %>% predict(train_data, train_targets, batch_size = batch_size)
